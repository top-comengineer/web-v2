			...
			<Tab.Group>
				<Tab.List className={'flex flex-row w-full'}>
					<Tab
						...
						<p className={'text-lg text-center text-inherit'}>{'Add liquidity'}</p>
					</Tab>
					<Tab
						className={({selected}): string => `hover:bg-grey-4 focus:outline-none w-full h-20 border-b-4 flex-center cursor-pointer p-0 ${selected ? 'border-black font-bold text-black' : 'text-grey-2 border-0 border-grey-3 transition-colors font-normal'}`}>
						<p className={'text-lg text-center text-inherit'}>{'Withdraw liquidity'}</p>
					</Tab>
					<Tab
						className={({selected}): string => `hover:bg-grey-4 focus:outline-none w-full h-20 border-b-4 flex-center cursor-pointer p-0 ${selected ? 'border-black font-bold text-black' : 'text-grey-2 border-0 border-grey-3 transition-colors font-normal'}`}>
						<p className={'text-lg text-center text-inherit'}>{'Manage directly'}</p>
					</Tab>
				</Tab.List>
